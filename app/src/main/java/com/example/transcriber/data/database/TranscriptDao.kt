...
interface TranscriptDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insertSegments(segments: List<com.example.transcriber.data.model.TranscriptSegmentEntity>): List<Long>

    @Query("SELECT * FROM transcript_segments WHERE transcriptId = :transcriptId ORDER BY startMs ASC")
    fun observeSegmentsByTranscriptId(transcriptId: Long): Flow<List<com.example.transcriber.data.model.TranscriptSegmentEntity>>

    @Query("SELECT * FROM transcript_segments WHERE transcriptId = :transcriptId ORDER BY startMs ASC")
    suspend fun getSegmentsByTranscriptId(transcriptId: Long): List<com.example.transcriber.data.model.TranscriptSegmentEntity>

    @Query("SELECT * FROM transcript_segments WHERE transcriptId = :transcriptId ORDER BY startMs ASC")
    suspend fun getSegmentsOnce(transcriptId: Long): List<com.example.transcriber.data.model.TranscriptSegmentEntity>
